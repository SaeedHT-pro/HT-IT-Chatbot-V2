...
# --- PROMPT TEMPLATES ---
# (Keep your existing prompt templates as they are)
...
- "SOP_Store": For detailed procedures, troubleshooting specific hardware/software product issues, technical specifications, component details, or any information typically found in user manuals, standard operating procedures, technical guides, or official notices.
...
1.  **Prioritize Relevance:** If the context contains information relevant to the user's query, synthesize it into a concise and helpful answer.
2.  **Address Specificity:** If the user asks for a *specific section, notice, or type of document* (e.g., "Liquid crystal display (LCD) notice", "Notice for built-in rechargeable battery") and the context *does not explicitly contain that exact section title or document type*, clearly state that the specific "notice" or "section" was not found.
3.  **Provide Available Information:** *Crucially*, even if the specific "notice" or "section" is not found, if the context *does* contain general information related to the query (e.g., general LCD troubleshooting, battery care tips), you MUST still provide that available relevant information to the user. Do not simply say "not found" if related information exists.
4.  **Insufficient Context:** If the context is genuinely insufficient or contains no relevant information at all, then politely state that you couldn't find specific information for that query.
5.  **No Fabrication:** Do not make up information.
6.  **Source Mention:** If the context is from a web search, you can optionally mention that.
...
def load_faqs(file_path="data/faqs/faq_data.xlsx"):
    ...
    try:
        ...
        # Ensure correct case for column names based on your updated prompt (Question, Answer)
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(ref_link) and ref_link:
                content += f"\nReference Link: {str(ref_link)}" # Ensure ref_link is string
                metadata["reference_link"] = str(ref_link)
            docs.append(Document(page_content=content, metadata=metadata))
        logger.info(f"Successfully loaded {len(docs)} FAQs.")
    ...
    for filename in os.listdir(sops_dir):
        ...
        try:
            ...
            if loader:
                ...
                logger.debug(f"Loaded {len(loaded_docs_for_file)} pages/parts from {filename}.")
        except Exception as e:
